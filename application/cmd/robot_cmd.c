...
static Publisher_t *upper_cmd_pub;           // 机械臂控制消息发布者
static Subscriber_t *upper_feed_sub;         // 机械臂反馈信息订阅者
static Upper_Ctrl_Cmd_s upper_cmd_send;      // 传递给机械臂的控制信息
static Upper_Upload_Data_s upper_fetch_data; // 从机械臂获取的反馈信息
#endif
...
static void RemoteControlSet()
{
	...
	if(switch_is_down(rc_data[TEMP].rc.switch_left) && switch_is_down(rc_data[TEMP].rc.switch_right))
		...
		chassis_cmd_send.chassis_mode = CHASSIS_FREE_MOVE;
	
	#ifdef ROBOT_ENGINEER	// 测试
	if(switch_is_up(rc_data[TEMP].rc.switch_left) && switch_is_up(rc_data[TEMP].rc.switch_right))
		upper_cmd_send.valve_mode = valve1_on | valve2_on | valve3_on | valve4_on;
	else
		upper_cmd_send.valve_mode = vlave_off;
	upper_cmd_send.upper_mode = UPPER_TEST;
	#endif
...
void RobotCMDTask()
{
	...
	CalcOffsetAngle();
#endif
	// 根据遥控器左侧开关,确定当前使用的控制模式为遥控器调试还是键鼠	//待完善
